...
\begin{exercise}{Algorithmenanalyse}
  Analysieren Sie den angegebenen Pseudocode (\ref{alg:somesort}) zur Sortierung der Liste $A$ durch den Aufruf \textalgo{somesort($A, 0, A.\text{length}$)}:
  ...
  \begin{solution}
    \begin{enumerate}
      ...
      \item Ein Sortierverfahren ist stabil, wenn die Reihenfolge von Elementen mit gleichem Schlüsselwert erhalten bleibt. \textalgo{somesort} ist nicht stabil, da in Zeile 6 und 7 das größere Element an das Ende der Liste gesetzt wird, ohne zu prüfen, ob es gleich groß ist wie das andere Element.
    \end{enumerate}
  \end{solution}
\end{exercise}
