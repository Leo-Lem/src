...
\begin{exercise}{Algorithmen}
  \begin{enumerate}
    \item Betrachten Sie folgenden Algorithmus in ihrer aus der Vorlesung bekannten Implementierung. Wählen Sie geeignete Parameter zur Beschreibung der Eingabegröße und nutzen Sie diese, um eine möglichst gute obere Schranke an die asymptotische Laufzeitkomplexität (worst-case) anzugeben.
          \hint{Beispiel \textalgo{prim}: $n$ Anzahl Knoten, $m$ Anzahl Kanten $\to$ Laufzeit $\mathcal{O}(n \cdot \log^2(n) + m \cdot \log(n))$}
          \begin{itemize}
            \item \textalgo{quicksort}
            \item \textalgo{countingsort}
            \item \textalgo{dijkstra} (single-source shortest path)
            \item \textalgo{floydWarshall} (all-pairs shortest path)
          \end{itemize}
...
\begin{exercise}{Heapsort}
  Das Array $A = [2,3,4,9,6,5]$ soll mit Hilfe des in der Vorlesung vorgestellten \algref{heapsort} Algorithmus aufsteigend sortiert werden. Geben Sie den Inhalt des Arrays $A$ nach jedem Schritt des Algorithmus an.
  \input{res/heapsort.alg}
\end{exercise}
...
\begin{exercise}{Who Am I?}
  Betrachten Sie den \algrefp{whoAmI}.
  \input{res/whoami.alg}
  \begin{enumerate}
    \item Geben Sie den Rückgabewert des Aufrufs \textalgo{whoAmI}(1,6) sowie den Inhalt der Variablen $a, b, n$ \& $m$ am Ende des Algorithmus an.
